...
def incalibrate_imu(imu, calibration_samples, dt):
    """
    Extended calibration function to estimate gyro biases,
    initial Euler angles and gyro noise parameters (error and bias drift)
    assuming the quadcopter is stationary.

    Returns:
      bx, by, bz: gyro bias estimates
      phi_bias, theta_bias, psi_bias: initial Euler angle estimates
      w_gyro_error_est: estimated gyro noise error factor (mean std deviation)
      w_gyro_bias_drift_est: estimated gyro bias drift factor (set to 0.1)
    """
    bx, by, bz = 0.0, 0.0, 0.0
    phi_sum, theta_sum, psi_sum = 0.0, 0.0, 0.0
    gyro_readings = []
    ...
    for _ in range(calibration_samples):
        ...
        # Save gyro readings for noise estimation
    ...
    # Estimate gyro noise error as the mean standard deviation across axes
    gyro_readings = np.array(gyro_readings)
    std_dev = np.std(gyro_readings, axis=0)
    w_gyro_error_est = np.mean(std_dev)
    # Set bias drift estimate as desired (here fixed to 0.1)
    w_gyro_bias_drift_est = 0.1

    return bx, by, bz, phi_bias, theta_bias, psi_bias, w_gyro_error_est, w_gyro_bias_drift_est
...
def read_imu_data_raw(imu, dt, prev_gyro_phi, prev_gyro_theta, prev_psi):
    ...
    # Integrate gyro for reference (raw integration, will be replaced by corrected integration later)
...
if __name__ == '__main__':
    ...
    # -- Calibrate both IMUs using extended calibration to also estimate gyro noise parameters
    bx1, by1, bz1, phi_bias1, theta_bias1, psi_bias1, w_gyro_error1, w_gyro_bias_drift1 = incalibrate_imu(imu1, calibration_samples, Ts)
    bx2, by2, bz2, phi_bias2, theta_bias2, psi_bias2, w_gyro_error2, w_gyro_bias_drift2 = incalibrate_imu(imu2, calibration_samples, Ts)
    bx = (bx1 + bx2) / 2
    by = (by1 + by2) / 2
    bz = (bz1 + bz2) / 2
    ...
    w_gyro_error = (w_gyro_error1 + w_gyro_error2) * 2
    w_gyro_bias_drift = (w_gyro_bias_drift1 + w_gyro_bias_drift2) * 2
    ...
    print(f"Estimated Gyro Error: {w_gyro_error:.4f} rad/s, "
          f"Estimated Gyro Bias Drift: {w_gyro_bias_drift:.4f}")
    ...
    # The following assignments are now obtained from calibration:
    # w_gyro_error = 0.05  (approximately) and w_gyro_bias_drift = 0.1
    #w_gyro_error = 0.05
    ...
    # Track integrated gyro angles for reference (initialize with biases)
    prev_phi_gyro   = phi_bias
    prev_theta_gyro = theta_bias
    prev_psi_gyro   = psi_bias

    # -----------------------------------------------------------------
    # NEW: Initialize gyro bias estimate for drift compensation (x,y,z)
    gyro_bias_est = np.array([bx, by, bz])
    # -----------------------------------------------------------------

    print("Starting IMU Tuning (Madgwick + Complementary with Gyro Drift Compensation)...")
    ...
    for i in range(num_samples):
        ...
        # -----------------------------------------------------------------
        # NEW: Gyro error modeling and drift compensation
        # Subtract the current bias estimate from the averaged gyro measurements.
        gyr_corrected = np.array(gyro_aligned) - gyro_bias_est

        # Use the bias-corrected gyro to update integrated angles
