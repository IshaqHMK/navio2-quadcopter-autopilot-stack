...
from imu.wmm import WMM
wmm = WMM(latitude=25.312854, longitude=55.490652, height=30.825)
print(wmm.magnetic_elements)  # Print all magnetic elements from the WMM model
...
def quaternion_to_euler(q):
    ...
    sinp = 2*(w*y - z*x)
    sinp = max(-1.0, min(1.0, sinp))  # Clamp to [-1, 1] to avoid numeric errors
...
def complementary_update(q_prev, gyr, acc, mag, dt, gain):
    ...
    gyr: gyro measurement in rad/s (list or array of length 3)
    acc: accelerometer measurement in m/s^2 (list or array of length 3)
    mag: magnetometer measurement (list or array of length 3)
...
def _compute_tilt_compensated_yaw(mx, my, mz, roll_rad, pitch_rad):
    """
    Compute yaw (heading) from magnetometer data (mx, my, mz) with tilt compensation.
    
    Reference:
      - Madgwick, S. (2010). "An efficient orientation filter for inertial 
        and inertial/magnetic sensor arrays." 
      - Also see NXP's "AN4248" application note for a similar derivation.
    """

    # Example formula that *ignores* tilt compensation if you just do:
    yaw_rad = math.atan2(my, mx)

    # If you actually want tilt compensation, see:
    # Bfx = mx*math.cos(pitch_rad) + mz*math.sin(pitch_rad)
    # Bfy = (mx*math.sin(roll_rad)*math.sin(pitch_rad)
    #        + my*math.cos(roll_rad)
    #        - mz*math.sin(roll_rad)*math.cos(pitch_rad))
    # yaw_rad = math.atan2(-Bfy, Bfx)
...
def calibrate_imu(imu, calibration_samples, dt):
    ...
    for i in range(calibration_samples):
        ...
        mag_raw[1] = -mag_raw[1] * 0
        mag_raw[2] = -mag_raw[2] * 0
...
def read_imu_data_raw(imu, dt, prev_gyro_phi, prev_gyro_theta, prev_psi):
    ...
    Read raw IMU data.
    Returns:
      acc_comp: Accelerometer measurement (list)
      gyro_aligned: Gyro measurement (list)
      mag_aligned: Magnetometer measurement (list)
...
def read_dual_imu_data_raw(imu1, imu2, dt, prev_gyro_phi, prev_gyro_theta, prev_psi):
    ...
    # Average the accelerometer and gyro measurements element-wise.
    acc_comp = list((np.array(data1[0]) + np.array(data2[0])) / 2)
    gyro_aligned = list((np.array(data1[1]) + np.array(data2[1])) / 2)
    # For magnetometer, here we use the first IMU's data.
    mag_aligned = data1[2]
...
if __name__ == '__main__':
    ...
    # -------------------------------------------------
    # ADD Madgwick Gains from Madgwick 2010 (Eqn. 50/51)
    # -------------------------------------------------
    # These are example values; you should tune w_gyro_error & w_gyro_bias_drift
    # based on your sensor’s typical noise and drift characteristics.
    w_gyro_error = 0.05         # rad/s (estimated average gyro error)
    w_gyro_bias_drift = 0.001   # rad/s (estimated gyro bias drift rate)

    Beta = math.sqrt(3.0/4.0) * w_gyro_error     # eqn. (50)
    Zeta = math.sqrt(3.0/4.0) * w_gyro_bias_drift  # eqn. (51)

    # Initialize filters with the computed gains:
    madgwick = Madgwick(
        beta=Beta,                  # Filter gain for mean zero gyro errors
        zeta=Zeta,                  # Filter gain for gyro bias drift (if supported in your class)
        magnetic_ref=[33554.525176223426, 1297.2515142936088, 28144.041137805445]
    )
    madgwick.q0 = q0_madw
    ...
    ekf = EKF(
        ...
        mag=np.zeros((1, 3)),
        magnetic_ref=[33554.525176223426, 1297.2515142936088, 28144.041137805445],
        noises=[(0.9)**2, (0.001)**2, (0.01)**2]
    )
    ekf.q0 = q0_ekf

    # For the complementary filter, we use our own quaternion
    q_comp = q0_comp.copy()
    ...
    num_samples = int(120 / Ts)  # Run for 120 seconds at Ts intervals
    start_time = time.time()
    ...
    for i in range(num_samples):
        ...
        # Update integrated gyro angles for reference
        prev_phi_gyro = phi_gyro
        prev_theta_gyro = theta_gyro
        prev_psi_gyro += psi_gyro

        # Update Madgwick filter
        q_madw = madgwick.updateMARG(
            madgwick.q0, gyr=gyro_aligned, acc=acc_comp, mag=mag_aligned, dt=Ts
        )
        madgwick.q0 = q_madw

        # Update EKF
        q_ekf = ekf.update(
            ekf.q0, gyr=gyro_aligned, acc=acc_comp, mag=mag_aligned, dt=Ts
        )
        ekf.q0 = q_ekf

        # Update Complementary filter
        q_comp = complementary_update(
            q_comp, gyr=gyro_aligned, acc=acc_comp, mag=mag_aligned, dt=Ts, gain=comp_gain
        )
        ...
        print(f"Time: {i*Ts:.2f}s | "
              f"Madw: Phi {np.rad2deg(phi_madw):.2f}°, Theta {np.rad2deg(theta_madw):.2f}°, Psi {np.rad2deg(psi_madw):.2f}° | "
    ...
    # Optionally, print standard deviations of each filter's output
    print(f"Madgwick Std: Phi {np.std(madw_phi_arr):.4f}, "
          f"Theta {np.std(madw_theta_arr):.4f}, Psi {np.std(madw_psi_arr):.4f} rad")
    print(f"EKF Std: Phi {np.std(ekf_phi_arr):.4f}, "
          f"Theta {np.std(ekf_theta_arr):.4f}, Psi {np.std(ekf_psi_arr):.4f} rad")
    print(f"Comp. Filter Std: Phi {np.std(comp_phi_arr):.4f}, "
          f"Theta {np.std(comp_theta_arr):.4f}, Psi {np.std(comp_psi_arr):.4f} rad")
...
if __name__ == '__main__':
    ...
    Beta = 0.033 #math.sqrt(3.0/4.0) * w_gyro_error     # eqn. (50)
    Zeta = 10.041 # math.sqrt(3.0/4.0) * w_gyro_bias_drift  # eqn. (51)

    # Initialize filters with the computed gains:
    madgwick = Madgwick(
        gain_marg=Beta,                  # Filter gain for mean zero gyro errors
        gain_imu=Zeta                  # Filter gain for gyro bias drift (if supported in your class)
    )
    madgwick.q0 = q0_madw
    ...
    for i in range(num_samples):
        ...
        #    madgwick.q0, gyr=gyro_aligned, acc=acc_comp, mag=mag_aligned, dt=Ts
        #)
        q_madw = madgwick.updateIMU(
            madgwick.q0, gyr=gyro_aligned, acc=acc_comp, dt=Ts
        )
        madgwick.q0 = q_madw
