...
# -*- coding: utf-8 -*-
"""
IMU Tuning Code for Complementary Filter Only
"""
...
comp_gain = 0.98
windowSize = 5  # ~2s if sampling at 100Hz
stationaryThreshold = 0.001  # rad/s standard deviation threshold
# ========================
# Third-Party / Custom Modules
# ========================
# (Removed Madgwick, EKF imports)
...
def complementary_update(q_prev, gyr, acc, dt, gain):
    ...
    - We keep yaw from gyro integration alone (no magnetometer).
    """
    ...
    # Yaw is purely gyro-based => psi_final = psi_g
...
###############################################################################
# Paper's Yaw Drift Removal (Ring Buffer) - as a small class or function
###############################################################################
class YawDriftCorrectorRingBuffer:
    """
    Implements the paper's adaptive sliding-window yaw drift removal
    using a ring buffer, mimicking the MATLAB code approach.
    """
    def __init__(self, window_size=200, stationary_threshold=0.001, init_offset=0.0):
        self.window_size = window_size
        self.stationary_threshold = stationary_threshold
        self.Db = init_offset  # drift offset
        self.buffer = np.zeros(window_size, dtype=float)
        self.rbIndex = 0
        self.samples_count = 0  # track how many samples we have

    def update(self, raw_z):
        """
        Given raw_z (rad/s), returns correctedZ = raw_z - Db,
        with the possibility of updating Db if we detect 'static.'
        """
        # (4) Update window: Insert currentZ into ring buffer
        ...
        # Only do motion determination once the window is "full"
        if self.samples_count >= self.window_size:
            ...
            if windowStd < self.stationary_threshold:
                # => STATIC => "Data set zero"
                self.Db = np.mean(self.buffer)  # Paper eqn(4)
                correctedZ = raw_z - self.Db
            else:
                # => MOTION => keep last Db
                correctedZ = raw_z - self.Db
        else:
            # Window not yet full => just use initial offset
            correctedZ = raw_z - self.Db
...
def read_imu_data_raw(imu, dt, prev_gyro_phi, prev_gyro_theta, prev_psi):
    ...
    # Euler from accelerometer alone
    phi_acc   = math.atan2(ay, az)
    theta_acc = math.atan2(-ax, math.sqrt(ay*ay + az*az))
...
if __name__ == '__main__':
    ...
    # Initialize Yaw Drift Corrector (ring-buffer method, no deque)
    yaw_corrector = YawDriftCorrectorRingBuffer(window_size=windowSize,
                                                stationary_threshold=stationaryThreshold,
                                                init_offset=0.0)

    print("Starting IMU Tuning (Complementary Only with Yaw Drift Correction)...")
    num_samples = int(50 / Ts)  # 50 seconds at 100 Hz
    start_time = time.time()
    ...
    for i in range(num_samples):
        ...
        # --- Apply Paper's Yaw Drift Correction on Z-axis
        # Replace the raw Z reading with its corrected value.
        gz_corrected = yaw_corrector.update(gyro_aligned[2])
        gyro_aligned[2] = gz_corrected
        ...
        print(f"Time: {i*Ts:.2f}s | Comp: R {np.rad2deg(phi_comp):.2f}°, P {np.rad2deg(theta_comp):.2f}°, Y {np.rad2deg(psi_comp):.2f}°")
    ...
    print(f"Complementary Std [rad]: phi={np.std(comp_phi_arr):.4f}, theta={np.std(comp_theta_arr):.4f}, psi={np.std(comp_psi_arr):.4f}")
