...
# Global variable to hold magnetometer bias (set during calibration)
mag_bias_cal = None
...
def complementary_update(q_prev, gyr, acc, mag, dt, gain):
    ...
    # Fuse the yaw: gyro integration vs magnetometer-based yaw.
    psi_final = gain * psi_g + (1 - gain) * mag_yaw

    # --- 4) Complementary fuse for roll & pitch:
    phi_final   = gain * phi_g   + (1 - gain) * phi_acc
    theta_final = gain * theta_g + (1 - gain) * theta_acc

    # --- 5) Convert fused Euler angles back to quaternion.
...
def _compute_tilt_compensated_yaw(mx, my, mz, roll_rad, pitch_rad):
    ...
    """
    # Adjust the magnetometer measurements with the tilt (roll & pitch)
    # Reference: https://cache.freescale.com/files/sensors/doc/app_note/AN4248.pdf
...
def calibrate_imu(imu, calibration_samples, dt):
    ...
    for _ in range(calibration_samples):
        ...
        # Accumulate magnetometer readings for bias estimation
        mag_sum += mag_raw
        ...
        # Rough yaw from magnetometer (without tilt compensation)
...
def read_imu_data_raw(imu, dt, prev_gyro_phi, prev_gyro_theta, prev_psi):
    ...
    mag_raw[1] = -mag_raw[1]
    mag_raw[2] = -mag_raw[2]
    # Apply magnetometer bias correction if available
    global mag_bias_cal
...
if __name__ == '__main__':
    ...
    # -- Calibrate both IMUs (including magnetometer bias)
    bx, by, bz, phi_bias, theta_bias, psi_bias, mag_bias = calibrate_dual_imu(
        imu1, imu2, calibration_samples, Ts
    )
    # Store magnetometer bias globally for subsequent corrections
    ...
    # Initialize gyro bias estimate for drift compensation (x,y,z)
    ...
    for i in range(num_samples):
        ...
        # Gyro error modeling and drift compensation
        ...
        # Update Complementary filter using corrected gyro and magnetometer data
