...
def complementary_update(q_prev, gyr, acc, mag, dt, gain):
    ...
    - We keep yaw from gyro integration alone (mag is ignored).
    """
...
if __name__ == '__main__':
    ...
    print("Starting IMU Tuning (Complementary Only)...")
    num_samples = int(50 / Ts)  # e.g., 50 seconds at 100 Hz
    start_time = time.time()
    ...
    for i in range(num_samples):
        ...
        print(f"Time: {i*Ts:.2f}s | "
              f"Comp: R {np.rad2deg(phi_comp):.2f}°, "
              f"P {np.rad2deg(theta_comp):.2f}°, "
              f"Y {np.rad2deg(psi_comp):.2f}°")
    ...
    # Print standard deviations for Complementary
