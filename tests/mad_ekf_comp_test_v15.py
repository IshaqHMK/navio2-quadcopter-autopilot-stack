...
if __name__ == '__main__':
    ...
    # Define IMU offset (in meters) from the center of gravity: [X_imu, Y_imu, Z_imu]
    # Change these values to match your sensor’s displacement.
    IMU_OFFSET = np.array([-0.006, -0.005, 0.0])  # sensor displacement in meters (-6mm in X, -5mm in Y)

    # Initialize previous gyro measurement (for angular acceleration estimation)
    prev_gyr = np.array([0.0, 0.0, 0.0])
    ...
    q0_init = euler_to_quaternion(phi_bias, theta_bias, 0)
    q_madw = q0_init.copy()
    q_comp = q0_init.copy()

    w_gyro_error = 0.05
    w_gyro_bias_drift = 0.1
    Beta = 0.03
    ...
    for i in range(num_samples):
        ...
        # Compute angular acceleration (alpha) from gyro measurements
        alpha = (gyr_corrected - prev_gyr) / Ts  # α ≈ Δω/Δt
        # Compute rotational correction using the formula:
        # a_CG = a_imu - (α × r + ω × (ω × r))
        rotational_correction = np.cross(alpha, IMU_OFFSET) + np.cross(gyr_corrected, np.cross(gyr_corrected, IMU_OFFSET))
        # Correct the accelerometer measurement to the center of gravity
        acc_corrected = np.array(acc_comp) - rotational_correction

        prev_phi_gyro   = phi_gyro_corr
        prev_theta_gyro = theta_gyro_corr
        prev_psi_gyro   = psi_gyro_corr

        q_madw_new = madgwick.updateIMU(madgwick.q0, gyr_corrected.tolist(), acc_corrected.tolist(), Ts)

        madgwick.q0 = q_madw_new
        q_madw = q_madw_new

        q_comp = complementary_update(
            q_comp, gyr=gyr_corrected.tolist(), acc=acc_corrected.tolist(), mag=mag_aligned,
            dt=Ts, gain=comp_gain
        )

        prev_gyr = gyr_corrected.copy()
    ...
    save_path = f"/home/pi/Documents/Quadcopter_Control_v2/data/results/imu_yaw_{timestamp}.mat"
