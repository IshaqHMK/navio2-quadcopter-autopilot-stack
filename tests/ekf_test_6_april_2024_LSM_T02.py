...
if __name__ == '__main__':
    ...
    for i in range(num_samples):
        ...
        gyro_tol = 0.05 # 90% of max
   
        for j in range(3):
            upper_bound = gyro_max[j] * gyro_tol  # apply tolerance to max value
            lower_bound = gyro_min[j] * gyro_tol # apply tolerance to min value
            if lower_bound <= m9g[j] <= upper_bound:
                m9g[j] = gyro_bias[j]
