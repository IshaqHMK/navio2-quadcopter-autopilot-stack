...
class CustomEKF:
    def __init__(self, q0=None):
        ...
        gyroNoiseVar = 1e-5 #1e-5  # 1e-2 or 1e-5, tune as needed
        biasNoiseVar = 1e-12 #1e-12
    ...
    def update(self, q0, gyr, acc, mag, dt):
        ...
        # ------------------------------------------------------------------
        # CHANGED: Replaced the RK4 integration with a single-step (Euler)
        # integration so that the process model and its Jacobian align.
        # ------------------------------------------------------------------
        def T(phi_, theta_):
            return np.array([
                [1.0, math.sin(phi_)*math.tan(theta_), math.cos(phi_)*math.tan(theta_)],
                [0.0, math.cos(phi_),               -math.sin(phi_)],
                [0.0, math.sin(phi_)/math.cos(theta_), math.cos(phi_)/math.cos(theta_)]
            ])

        # Euler integration of angles:
        angle_rates = T(phi, theta).dot(np.array([p_corr, q_corr, r_corr]))
        phi_dot   = angle_rates[0]
        theta_dot = angle_rates[1]
        psi_dot   = angle_rates[2]

        phi_gyro   = phi   + dt * phi_dot
        theta_gyro = theta + dt * theta_dot
        psi_gyro   = psi   + dt * psi_dot
        ...
        # This Jacobian matches the single-step (Euler) approximation.
        # ------------------------------------------------------------------
        tan_theta = math.tan(theta)
        sec2_theta = 1.0 / (math.cos(theta)**2)
        F = np.eye(6)

        # We replace (q - bq) by q_corr and (r - br) by r_corr:
        # d(phi)/d(phi), d(phi)/d(theta), etc.
        F[0,0] = 1.0 + dt * ( math.cos(phi)*tan_theta*q_corr - math.sin(phi)*tan_theta*r_corr )
        ...
        F[2,0] = dt * ( (math.cos(phi)/math.cos(theta))*q_corr
                        - (math.sin(phi)/math.cos(theta))*r_corr )
        ...
        # Measurement Jacobian H (only phi and theta affect accelerometer measurement)
        H = np.zeros((3,6))
        ...
        if abs(np.linalg.norm(acc) - self.g) > self.gating_threshold:
            ...
            x_updated[3] += dt * self.k_phi_bias * (phi_acc - phi_gyro)
            x_updated[4] += dt * self.k_theta_bias * (theta_acc - theta_gyro)
...
if __name__ == '__main__':
    ...
    num_samples = int(10 / Ts)  # Run for 60 seconds
