...
and external integration for roll, pitch, yaw using Simpson's integration.
...
if __name__ == '__main__':
    ...
    # 2) 5-second pre-run to auto-tune drift threshold for all axes
    ...
    # Initialize Simpson's integration buffers for each axis
    simpson_buffer_x = []
    simpson_buffer_y = []
    simpson_buffer_z = []
    ...
    for i in range(num_main):
        ...
        # 5b) Simpson's Integration for improved integration
        # Append current corrected values to Simpson buffers
        simpson_buffer_x.append(gx_corr)
        simpson_buffer_y.append(gy_corr)
        simpson_buffer_z.append(gz_corr)
        # If we have 3 samples, integrate over them using Simpson's 1/3 rule
        if len(simpson_buffer_x) == 3:
            delta_phi = Ts/3 * (simpson_buffer_x[0] + 4*simpson_buffer_x[1] + simpson_buffer_x[2])
            delta_theta = Ts/3 * (simpson_buffer_y[0] + 4*simpson_buffer_y[1] + simpson_buffer_y[2])
            delta_psi = Ts/3 * (simpson_buffer_z[0] + 4*simpson_buffer_z[1] + simpson_buffer_z[2])
            phi_gyro += delta_phi
            theta_gyro += delta_theta
            psi_gyro += delta_psi
            # Retain the last sample for next Simpson group
            simpson_buffer_x = [simpson_buffer_x[2]]
            simpson_buffer_y = [simpson_buffer_y[2]]
            simpson_buffer_z = [simpson_buffer_z[2]]
        # Otherwise, if not enough samples, do nothing (wait for a full set)

        # 5c) Minimal complementary => fuse roll/pitch with accelerometer
        #     Keep yaw from our integrated psi_gyro
    ...
    print(f"Complementary Std [rad]: "
          f"phi={np.std(phi_comp_arr):.4f}, "
          f"theta={np.std(theta_comp_arr):.4f}, "
          f"psi={np.std(psi_comp_arr):.4f}")
