...
# -*- coding: utf-8 -*-
"""
IMU Tuning Code for Madgwick & EKF Filters with Sensor Offset and Misalignment Correction
Version: IMU-Tuning-vDualFilters-v1
"""
...
misalign_theta = math.radians(0)   # for example, 3.4° about Y-axis
...
def calibrate_imu(imu, calibration_samples, dt):
    ...
    for i in range(calibration_samples):
        ...
        # Sign corrections for magnetometer if needed.
        mag_raw[1] = -mag_raw[1]
        mag_raw[2] = -mag_raw[2]
...
# ------------------------
# New: Read Raw IMU Data Function
# ------------------------
def read_imu_data_raw(imu, dt, prev_gyro_phi, prev_gyro_theta, prev_psi):
    ...
    Returns:
      acc_comp: accelerometer reading corrected to the CG (as a list)
      gyro_aligned: misalignment-corrected gyro (as a list)
      mag_aligned: misalignment-corrected magnetometer (as a list)
      phi_acc, theta_acc: accelerometer-based roll and pitch
    ...
    # Compensation: a_cg = a_sensor - (domega x r + omega x (omega x r))
...
# ------------------------
# Quaternion/Euler Conversion Functions (unchanged)
# ------------------------
...
if __name__ == '__main__':
    ...
    q0_madw = euler_to_quaternion(phi_bias, theta_bias, psi_bias)
    q0_ekf = q0_madw.copy()
    ...
    print("Starting IMU Tuning (Both Filters)...")
    start_time = time.time()
    ...
    for i in range(num_samples):
        ...
        # Update both filters using the same sensor data.
        ...
        # Optional: display real-time measurements.
        print(f"Time: {i*Ts:.2f}s | Madw: Phi {np.rad2deg(phi_madw):.2f}°, Theta {np.rad2deg(theta_madw):.2f}°, Psi {np.rad2deg(psi_madw):.2f}° | EKF: Phi {np.rad2deg(phi_ekf):.2f}°, Theta {np.rad2deg(theta_ekf):.2f}°, Psi {np.rad2deg(psi_ekf):.2f}°")
