...
# -*- coding: utf-8 -*-
"""
IMU Tuning Code for Madgwick, Complementary, and EKF Filters 
Version: IMU-Tuning-vDualFiltersPlusCompPlusEKF-v1
MAIN VERSION
"""
...
if __name__ == '__main__':
    ...
    # -- Setup EKF Filter (using initial zeros and provided noise values)
    ekf = EKF(
        gyr=np.zeros((1, 3)),
        acc=np.zeros((1, 3)),
        mag=np.zeros((1, 3)),
        noises=[(0.1)**2, (0.1)**2, (0.1)**2]
    )
    ekf.q0 = q0_init.copy()
    ...
    # Arrays to store sensor measurements for plotting
    ...
    print("Starting IMU Tuning (Madgwick + Complementary + EKF)...")
    num_samples = int(50 / Ts)  # run ~120 seconds
    start_time = time.time()
    ...
    for i in range(num_samples):
        ...
        # Store sensor data measurements for later plotting
        ...
        # --- Update EKF filter
        q_ekf = ekf.update(ekf.q0, gyr=gyro_aligned, acc=acc_comp, dt=Ts)
        ekf.q0 = q_ekf

        # Convert all filters to Euler angles
        ...
        print(f"Time: {i*Ts:.2f}s | "
              ...
              f"EKF: R {np.rad2deg(phi_ekf):.2f}°, P {np.rad2deg(theta_ekf):.2f}°, Y {np.rad2deg(psi_ekf):.2f}° | "
