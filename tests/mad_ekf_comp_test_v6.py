...
def quaternion_to_euler(q):
    ...
    sinp = 2*(w*y - z*x)
    sinp = max(-1.0, min(1.0, sinp))  # Clamp sinp to [-1,1] to avoid numerical errors.
...
def _compute_tilt_compensated_yaw(mx, my, mz, roll_rad, pitch_rad):
    ...
    # 1) Tilt-compensate the magnetometer readings.
    #    Depending on the sign conventions and the exact roll/pitch definitions,
    #    you may need to adjust signs or reorder terms. The version below works
    #    for a common aerospace convention (roll around X, pitch around Y, yaw around Z).
    
    # Bfx: magnetometer X after tilt compensation
    Bfx = mx * math.cos(pitch_rad) + mz * math.sin(pitch_rad)
    
    # Bfy: magnetometer Y after tilt compensation
    Bfy = (mx * math.sin(roll_rad) * math.sin(pitch_rad)
           + my * math.cos(roll_rad)
           - mz * math.sin(roll_rad) * math.cos(pitch_rad))
    
    # 2) Compute yaw using atan2 of the compensated y and x.
    #    Negative sign on Bfy is common in some references (depending on axes).
...
if __name__ == '__main__':
    ...
    # Initialize filters.
    madgwick = Madgwick(beta=0.3, magnetic_ref=[33554.525176223426, 1297.2515142936088, 28144.041137805445])
    madgwick.q0 = q0_madw

    ekf = EKF(gyr=np.zeros((1, 3)), acc=np.zeros((1, 3)), mag=np.zeros((1, 3)), magnetic_ref=[33554.525176223426, 1297.2515142936088, 28144.041137805445],
    ...
    num_samples = int(120 / Ts)  # Run for 500*Ts seconds.
    start_time = time.time()
