...
total_simulation_time = 30 #200 # Simulation time in seconds  
...
def main():
    ...
    # theta
    adaptive_theta_control  = AdaptiveAttitudeController(lmbda=0.1,
                                    gamma1=0.1,
                                    gamma2=0.1,
                                    gamma3=0.1,
                                    K=0.1,
                                    ...
                                    B_init=0,
                                    d_init=0,
                                    J_min   = 0.5 * Jy,   # lower bound on J_hat
                                    J_max   = 1.5 * Jy,   # upper bound on J_hat
                                    B_min   = -0.1,        # lower bound on B_hat
                                    B_max   = 0.1,        # upper bound on B_hat
                                    d_min   = -0.05,      # lower bound on d_hat
                                    d_max   =  0.05 )      # upper bound on d_hat)
    # psi
    ...
    for i in range(num_samples):
        ...
        # 
        ### ----------------- Adaptive Controller --------------- ###
        _, J_hat_phi, B_hat_phi, d_hat_phi = adaptive_phi_control.update(
            meas = phi_meas, meas_dot = phi_dot_meas,
            des = phi_des, des_dot = phi_dot_des, des_ddot = phi_ddot_des, 
            Ts = Ts )   
        
        _, J_hat_theta, B_hat_theta, d_hat_theta = adaptive_theta_control.update(
    ...
    save_to_matfile(storage, gains, path_prefix='/home/pi/Documents/Quadcopter_Control_v2/data/results/quad_cross_PID_')
