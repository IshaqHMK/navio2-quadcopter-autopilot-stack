...
class AdaptiveGains:
    def __init__(self, initial_Kp, initial_Ki, initial_Kd, alpha=0.1, gamma=0.1, 
        ...
        self.vKi = 0.0
        self.vKd = 0.0
        self.last_grad_y_u = None         # store last valid grad_y_u
    ...
    def update(self, error, control, output, dt, desired=None, measured_rate=None):
        """
        ...
        Args:
            ...
            dt            : Time step.
            desired       : (Optional) The current desired reference value (e.g., desired angle).
            measured_rate : (Optional) The measured rate (e.g., sensor-based angular rate) for that axis.
        ...
        # If desired and measured_rate are provided, compute the derivative as:
        #   derivative = ( (desired - prev_desired)/dt ) - measured_rate
        if desired is not None and measured_rate is not None:
            if self.prev_desired is None:
                desired_dot = 0.0
            else:
                desired_dot = (desired - self.prev_desired) / dt
            derivative = desired_dot - measured_rate
            self.prev_desired = desired
        else:
            # Fall back to the standard finite-difference derivative of error.
        ...
        if self.prev_control is not None and abs(control - self.prev_control) > 1e-6:
            ...
            self.last_grad_y_u = grad_y_u
        else:
            grad_y_u = self.last_grad_y_u if self.last_grad_y_u is not None else 0.0

        # For a cost function J = 0.5 * error^2, dJ/dy = -error.
...
def main():
    ...
    Phi_KP, Phi_KI, Phi_KD = 2.89902730617153,	2.87664449647876,	5# 2.73960236802160
    Theta_KP, Theta_KI, Theta_KD =  2.59576160012261,	2.86526841797161,	5 #2.76572697565200

    Phi_KP, Phi_KI, Phi_KD = 3,	2,	5 
    Theta_KP, Theta_KI, Theta_KD =  2,	2,	5  

    Phi_KP, Phi_KI, Phi_KD = 3.00269899104010,	2.20631060859735,	5.5
    Theta_KP, Theta_KI, Theta_KD =  3.00159287319997,	1.97514013225999,	5.5
    ...
    adaptive_phi = AdaptiveGains(initial_Kp=Phi_KP, initial_Ki=Phi_KI, initial_Kd=Phi_KD, alpha=0.03, gamma=0.90, bounds_Kp=(1,5), bounds_Ki=(1,5), bounds_Kd=(3,7))
    adaptive_theta = AdaptiveGains(initial_Kp=Theta_KP, initial_Ki=Theta_KI, initial_Kd=Theta_KD, alpha=0.03, gamma=0.90, bounds_Kp=(1,5), bounds_Ki=(1,5), bounds_Kd=(3,7))
    ...
    for i in range(num_samples):
        ...
        if i > 0:
            ...
            # For roll adaptive gains:
            Phi_KP, Phi_KI, Phi_KD = adaptive_phi.update(
                error_phi, p_desired_prev, phi_meas_prev, Ts,
                desired=phi_des,           # Current desired roll angle
                measured_rate=phi_dot_calc  # Measured roll rate from sensor/EKF
            )
            Theta_KP, Theta_KI, Theta_KD = adaptive_theta.update(
                error_theta,       # error = theta_des - theta_meas
                q_desired_prev,    # previous control signal for pitch
                theta_meas_prev,   # previous measured pitch (theta)
                Ts,
                desired=theta_des,         # current desired pitch angle
                measured_rate=theta_dot_calc  # measured pitch rate (from euler_angle_rates)
            )
        ...
        #Phi_KP, Phi_KI, Phi_KD = 3, 2, 5
        #Theta_KP, Theta_KI, Theta_KD = 2.5, 4, 3.5
        #Theta_KP, Theta_KI, Theta_KD = 3,2 , 5
