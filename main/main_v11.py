...
def reference_generator(time, initial_Z, initial_phi, initial_theta, initial_psi, signal=5):
    ...
    if signal == 1:
        ...
        if time < T: 
            ...
            phi_des = 10 * (np.pi / 180)  # Maintain roll at 0 degrees
            theta_des = 10 * (np.pi / 180)  # Maintain pitch at 0 degrees
            psi_des = 30 * (np.pi / 180)  # Step yaw to 30 degrees
        else:
            # Stabilize with roll and pitch at zero and yaw at 30 degrees
            phi_des = -10 * (np.pi / 180)  # Maintain roll at 0 degrees
            theta_des = -10 * (np.pi / 180)  # Maintain pitch at 0 degrees
            psi_des = 30 * (np.pi / 180)  # Maintain yaw at 30 degrees
...
# Initialize the MPU9250 IMU sensor
imu = mpu9250.MPU9250()
...
KT = 0.000021;  # Thrust force coeffecient (N/(rad/s)^2 ) (T-motor 12 inch prop)
...
Phi_KP, Phi_KI, Phi_KD = 0, 0, 0.0   
Theta_KP, Theta_KI, Theta_KD =  1, 0, 0.1   
...
def main():
    ...
    warm_up_start = total_delay - 3  # Begin warm-up 3 seconds before the end of the delay
