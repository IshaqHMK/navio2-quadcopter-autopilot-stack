...
def reference_generator(time, initial_Z, initial_phi, initial_theta, initial_psi, signal=5):
    ...
    if signal == 1:
        ...
        if time < T: 
            ...
            phi_des = 5 * (np.pi / 180)  # Maintain roll at 0 degrees
            theta_des = 5 * (np.pi / 180)  # Maintain pitch at 0 degrees
        ...
        """
        You originally had:
        1) from t in [0, T): half-cos from initial angles to 0
        2) from t in [T, hold_time): hold at 0
        3) from t in [hold_time, step_time): step psi to 30°, roll/pitch=0
        4) from t >= step_time: stay at 30° yaw forever

        NOW we insert:
        - a sinusoidal phase from t in [step_time, 130),
        - hold(0°) from [130, 140),
        - then stay at 0° forever beyond 140

        This maintains your original T, hold_time, step_time.
        """

        # Your original times
        T = 5                 # half-cos transition duration
        hold_time = T + 5     # => 10
        step_time = hold_time + 10  # => 20

        # Additional times for the new phase
        # We'll do sinusoidal from t in [step_time, 130)
        sinusoid_end = 130.0
        # Then hold(0) from [130, 140),
        final_hold_start = sinusoid_end
        final_hold_end   = sinusoid_end + 10.0  # => 140

        if time < T:
            # --- Phase 1: half-cos from initial angles to 0 over [0, 5)
            phi_des   = initial_phi   * 0.5 * (1.0 + np.cos(np.pi * time / T))
            theta_des = initial_theta * 0.5 * (1.0 + np.cos(np.pi * time / T))
            psi_des   = initial_psi   * 0.5 * (1.0 + np.cos(np.pi * time / T))

        elif time < hold_time:
            # --- Phase 2: hold(0) from t in [5, 10)
            phi_des   = 0.0
            theta_des = 0.0
            psi_des   = 0.0

        elif time < step_time:
            # --- Phase 3: step yaw to 30°, keep roll/pitch=0 from t in [10, 20)
            phi_des   = 0.0
            theta_des = 0.0
            psi_des   = 0 * (np.pi / 180.0)  # 30 deg

        elif time < sinusoid_end:
            # --- Phase 4 (NEW): do sinusoidal from t in [20, 130)
            # Example: small ±5 deg in roll/pitch, ±10 deg in yaw
            freq_phi   = 0.1  # Hz
            freq_theta = 0.1
            freq_psi   = 0.1
            amp_phi    = np.radians(2.0)
            amp_theta  = np.radians(5.0)
            amp_psi    = np.radians(2.0)
            
            # Time since we started the sinusoid
            time_in_sin = time - step_time

            phi_des   =  0*amp_phi   * np.sin(2.0 * np.pi * freq_phi   * time_in_sin)
            theta_des =  amp_theta * np.sin(2.0 * np.pi * freq_theta * time_in_sin)
            psi_des   =  0* amp_psi   * np.sin(2.0 * np.pi * freq_psi   * time_in_sin)

        elif time < final_hold_end:
            # --- Phase 5: hold(0) from t in [130, 140)
            phi_des   = 0.0
            theta_des = 0.0
            psi_des   = 0.0

        else:
            # --- Phase 6: remain at 0 forever from t >= 140
            phi_des   = 0.0
            theta_des = 0.0
            psi_des   = 0.0
...
def attitude_PID(Z_des_GF, phi_des, theta_des, psi_des, Z_meas, phi_meas, theta_meas, psi_meas, p_meas, q_meas, r_meas, 
    ...
    # --- Use the body rates & angles to compute Euler-angle rates ---
    ...
    phi_error_dot = 0.0 - phi_dot_calc
    ...
    theta_error_dot = 0.0 - theta_dot_calc
...
imu.initialize()
 ...
 # -------- Initialize Filters -------- #
filter_cutoff = 5.0  # Cutoff frequency in Hz
...
#Phi_KP, Phi_KI, Phi_KD = 10, 4, 2.5
#Theta_KP, Theta_KI, Theta_KD = 10,4, 2.75
...
# madgwick = Madgwick(gyr=m9g, acc=m9a, mag = m9m, gain=0.020, beta=0.25, frequency=1/Ts)  
# madgwick = Madgwick(gyr=m9g, acc=m9a, mag = m9m, gain=0.021, beta=0.17, frequency=1/Ts) 

# madgwick = Madgwick(gyr=m9g, acc=m9a, mag=m9m, gain_marg=0.20, frequency=1/Ts)  
# ekf = EKF(gyr=m9g, acc=m9a, mag=m9m, magnetic_ref=[17.06, 0.78, 34.39])
#ekf = EKF(gyr=m9g, acc=m9a, mag=m9m , frequency=1/Ts)  
ekf = EKF(gyr=m9g, acc=m9a, mag=m9m, noises=[(70e-3)**2, (0.244e-3)**2, (0.43e-3)**2])

#madgwick = Madgwick(gyr=m9g, acc=m9a, mag = m9m, gain=0.021, beta=0.26, frequency=1/Ts)   
#madgwick = Madgwick(gyr=m9g, acc=m9a, mag = m9m, beta=0.30, frequency=1/Ts)   
...
def main():
    ...
    Phi_KP, Phi_KI, Phi_KD = 10, 4, 2.5 
    Theta_KP, Theta_KI, Theta_KD = 12,4, 3.5
    ...
    adaptive_phi = AdaptiveGains(initial_Kp=Phi_KP, initial_Ki=Phi_KI, initial_Kd=Phi_KD, alpha=0.125, gamma=0.9, bounds_Kp=(0,15), bounds_Ki=(0,10), bounds_Kd=(1.5,4))
    adaptive_theta = AdaptiveGains(initial_Kp=Theta_KP, initial_Ki=Theta_KI, initial_Kd=Theta_KD, alpha=0.125, gamma=0.9, bounds_Kp=(0,15), bounds_Ki=(0,10), bounds_Kd=(1.5,4))
    ...
    while time.time() - start_no_speed_time < total_delay:
        ...
        X_ddot_cur, Y_ddot_cur, Z_ddot_cur, mx, my, mz, p_cur, q_cur, r_cur, phi_mf, theta_mf, psi_mf, phi_acc_cur, theta_acc_cur, phi_gyro_cur, theta_gyro_cur, psi_gyro_cur, q = read_imu_data(
            imu, Ts, prev_phi_gyro, prev_theta_gyro, prev_psi_gyro, ekf.q0, ekf)
        #
    ...
    for i in range(num_samples):
        ...
        phi_meas = apply_recursive_filter(phi_meas, filtered_phi, alpha, 5) # was 10
        theta_meas = apply_recursive_filter(theta_meas, filtered_theta, alpha, 5)
        psi_meas = apply_recursive_filter(psi_meas, filtered_psi, alpha, 5)

        #p_meas = apply_recursive_filter(p_meas, filtered_p_meas, alpha, 5) # was 10
        #q_meas = apply_recursive_filter(q_meas, filtered_q_meas, alpha, 5) # was 10
        #r_meas = apply_recursive_filter(r_meas, filtered_r_meas, alpha, 5) # was 10
