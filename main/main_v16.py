...
Phi_KP, Phi_KI, Phi_KD = 0.0, 0.0, 0
Theta_KP, Theta_KI, Theta_KD = 0, 0.0, 0
...
madgwick = Madgwick(gyr=m9g, acc=m9a, mag = m9m, gain=0.021, beta=0.32, frequency=1/Ts)  # chatgpt recomended  
...
def main():
    ...
    for i in range(num_samples):
        ...
        # Apply filtering to roll, pitch, and yaw ---------------------
        filtered_phi.append(phi_meas)
        filtered_theta.append(theta_meas)
        filtered_psi.append(psi_meas)

        if len(filtered_phi) > 1:
            filtered_phi[-1] = butter_lowpass_filter(filtered_phi, filter_cutoff, 1 / Ts, filter_order)[-1]
            filtered_theta[-1] = butter_lowpass_filter(filtered_theta, filter_cutoff, 1 / Ts, filter_order)[-1]
            filtered_psi[-1] = butter_lowpass_filter(filtered_psi, filter_cutoff, 1 / Ts, filter_order)[-1]

        # Use filtered angles for control
        phi_meas, theta_meas, psi_meas = filtered_phi[-1], filtered_theta[-1], filtered_psi[-1]
        #phi_meas, theta_meas  = filtered_phi[-1], filtered_theta[-1] 
