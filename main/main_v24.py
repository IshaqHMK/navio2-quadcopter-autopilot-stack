...
total_simulation_time = 120 # Simulation time in seconds  
...
def main():
    ...
    adaptive_phi = AdaptiveGains(initial_Kp=Phi_KP, initial_Ki=Phi_KI, initial_Kd=Phi_KD, alpha=0.1, gamma=0.9, bounds_Kp=(0.5,10), bounds_Ki=(0.5,10), bounds_Kd=(1,5))
    adaptive_theta = AdaptiveGains(initial_Kp=Theta_KP, initial_Ki=Theta_KI, initial_Kd=Theta_KD, alpha=0.1, gamma=0.9, bounds_Kp=(0.5,10), bounds_Ki=(0.5,10), bounds_Kd=(1,5))
    ...
    while time.time() - start_no_speed_time < total_delay:
        ...
        # ---------- measure IMU (before experimenting - EKF ) ----------
        m9a, m9g, m9m = imu.getMotion9()
        m9a = np.array(m9a)
        m9g = np.array(m9g)
        if prev_acc is None:
        ...
        # Interchange & assign to match our confg. and process (does NOT involve EKF):  
        X_ddot_meas, Y_ddot_meas, Z_ddot_meas =  m9a[0], m9a[1], m9a[2]  # Acc measurement x, y, z
        p_meas, q_meas, r_meas =  m9g[0], m9g[1], m9g[2]  # Gyro measurement x, y, z
        phi_meas, theta_meas, psi_meas = phi_ekf, theta_ekf, psi_ekf 
        # Note: No Interchanging needed! 8-4-2025
        # -----------------------------
        time.sleep(Ts)
    ...
    for i in range(num_samples):
        ...
        # Low pass filter:
        if i == 0:
            filtered_phi, filtered_theta, filtered_psi = phi_ekf, theta_ekf, psi_ekf
        else:
        ...
        phi_ekf, theta_ekf, psi_ekf = filtered_phi, filtered_theta, filtered_psi
