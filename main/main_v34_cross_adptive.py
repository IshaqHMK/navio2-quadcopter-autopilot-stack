...
# Note: copied from version 30
# adaptive controller for phi-theta-psi sim file v611 v32
...
# Reference Functions: 18 - 05 - 2025
def reference_generator(sim_time, initial_Z, initial_phi, initial_theta, initial_psi, dt, signal):
    """
    Generates reference signals for altitude and attitude, including double‐dot terms.
    ...
    if signal == 1:
        ...
        z_des        = A_Z/2 * (1 - np.cos(omega * t))
        z_dot_des    = A_Z/2 * (omega * np.sin(omega * t))
        z_ddot_des   = A_Z/2 * (omega**2 * np.cos(omega * t))

        phi_des      = A_phi/2 * (1 - np.cos(omega * t))
        phi_dot_des  = A_phi/2 * (omega * np.sin(omega * t))
        phi_ddot_des = A_phi/2 * (omega**2 * np.cos(omega * t))

        theta_des      = A_theta/2 * (1 - np.cos(omega * t))
        theta_dot_des  = A_theta/2 * (omega * np.sin(omega * t))
        theta_ddot_des = A_theta/2 * (omega**2 * np.cos(omega * t))

        psi_des      = A_psi/2 * (1 - np.cos(omega * t))
        psi_dot_des  = A_psi/2 * (omega * np.sin(omega * t))
        psi_ddot_des = A_psi/2 * (omega**2 * np.cos(omega * t))

    elif signal == 4:
        # φ smooth step
        A_phi = 1 * (np.pi / 180)
        if sim_time < t4_phi_start:
            phi_des = phi_dot_des = phi_ddot_des = 0.0
        elif sim_time < t4_phi_start + T:
            tau = sim_time - t4_phi_start
            cos_t = np.cos(np.pi * tau / T)
            sin_t = np.sin(np.pi * tau / T)
            phi_des      = A_phi * 0.5 * (1 - cos_t)
            phi_dot_des  = A_phi * (np.pi / (2 * T)) * sin_t
            phi_ddot_des = A_phi * (np.pi / (2 * T)) * (np.pi / T) * cos_t
        elif sim_time < t4_phi_end:
            phi_des = A_phi; phi_dot_des = phi_ddot_des = 0.0
        elif sim_time < t4_phi_end + T:
            tau = sim_time - t4_phi_end
            cos_t = np.cos(np.pi * tau / T)
            sin_t = np.sin(np.pi * tau / T)
            phi_des      = A_phi * 0.5 * (1 + cos_t)
            phi_dot_des  = -A_phi * (np.pi / (2 * T)) * sin_t
        ...
        if sim_time < t4_theta_start:
            theta_des = theta_dot_des = theta_ddot_des = 0.0
        elif sim_time < t4_theta_start + T:
            tau = sim_time - t4_theta_start
            cos_t = np.cos(np.pi * tau / T)
            sin_t = np.sin(np.pi * tau / T)
            theta_des      = A_theta * 0.5 * (1 - cos_t)
            theta_dot_des  = A_theta * (np.pi / (2 * T)) * sin_t
            theta_ddot_des = A_theta * (np.pi / (2 * T)) * (np.pi / T) * cos_t
        elif sim_time < t4_theta_end:
            theta_des = A_theta; theta_dot_des = theta_ddot_des = 0.0
        elif sim_time < t4_theta_end + T:
            tau = sim_time - t4_theta_end
            cos_t = np.cos(np.pi * tau / T)
            sin_t = np.sin(np.pi * tau / T)
            theta_des      = A_theta * 0.5 * (1 + cos_t)
            theta_dot_des  = -A_theta * (np.pi / (2 * T)) * sin_t
        ...
        z_des = z_dot_des = z_ddot_des = 0.0
        psi_des = psi_dot_des = psi_ddot_des = 0.0
    ...
    # ψ smooth step (signal 5)
        ...
        if sim_time < t5_start:
            ...
            cos_t = np.cos(np.pi * tau / T)
            sin_t = np.sin(np.pi * tau / T)
            psi_des      = A_psi * 0.5 * (1 - cos_t)
            psi_dot_des  = A_psi * (np.pi / (2 * T)) * sin_t
            psi_ddot_des = A_psi * (np.pi / (2 * T)) * (np.pi / T) * cos_t
        elif sim_time < t5_end:
            psi_des = A_psi; psi_dot_des = psi_ddot_des = 0.0
...
def motor_speed(U1, U2, U3, U4, KT, Kd, l, max_motor_speed, min_motor_speed):
    """
    Compute motor speeds for × geometry
    U1 : total thrust
    U2 : roll moment in × frame
    U3 : pitch moment in × frame
    U4 : yaw moment
    KT : thrust constant
    Kd : drag constant
    l  : arm length (to propeller) in plus frame
    max_motor_speed, min_motor_speed : limits in rad/s
    """
    # effective lever arm in × frame
    l_eff = l / math.sqrt(2.0)

    # Case 1
    w1_squared = U1/(4*KT) + ( U2 - U3)/(4*KT*l_eff) - U4/(4*Kd)
    w3_squared = U1/(4*KT) - ( U2 - U3)/(4*KT*l_eff) - U4/(4*Kd)
    w2_squared = U1/(4*KT) - ( U2 + U3)/(4*KT*l_eff) + U4/(4*Kd) 
    w4_squared = U1/(4*KT) + ( U2 + U3)/(4*KT*l_eff) + U4/(4*Kd)
 
    # enforce squared speed limits
    max_sq = max_motor_speed**2
    min_sq = min_motor_speed**2

    w1 = min(max(w1_squared, min_sq), max_sq)
    w2 = min(max(w2_squared, min_sq), max_sq)
    w3 = min(max(w3_squared, min_sq), max_sq)
    w4 = min(max(w4_squared, min_sq), max_sq)
...
def estimate_attitude_and_rates(imu, Ts, alpha_sensor, alpha_attitude, q0, custom_ekf, 
    ...
    angle_deg = 45
    ax, ay, az = rotate_z(m9a, angle_deg) # cross configuration
    gx, gy, gz = rotate_z(m9g, angle_deg) # cross configuration
    mx, my, mz = rotate_z(m9m, angle_deg) # cross configuration
    m9a, m9g, m9m = (ax, ay, az), (gx, gy, gz), (mx, my, mz) # for cross config
    m9a = np.array(m9a)
    m9g = np.array(m9g)
...
class AdaptiveAttitudeController:
    ...
    def __init__(self,
                 lmbda,      # filter bandwidth
                 gamma1,       # adaptation gain for J_hat
                 gamma2,       # adaptation gain for B_hat
                 gamma3,       # adaptation gain for D_hat
                 K,            # stabilizing gain on e_2
                 J_init,   # initial guess for J
                 B_init,   # initial guess for B
                 d_init,
                 J_min=None, J_max=None,    # bounds for J_hat
                 B_min=None, B_max=None,    # bounds for B_hat
                 d_min=None, d_max=None):   # bounds for d_hat
        ...
        self.e_int       = 0.0   # integration of e
        self.prev_e_dot  = 0.0   # last e_dot

    def update(self,
               meas,       # measured variable 
               meas_dot,   # measured rate 
               des,        # desired variable
               des_dot,    # desired rate
               des_ddot,   # desired accelration
               Ts):        # sample time
        
        # 1) compute error and its derivative
        e     = des      - meas
        e_dot = des_dot  - meas_dot

        # 2) update integral of e
        self.e_int += e * Ts 

        # 3) compute e1 and e2
        e1  = des_ddot + 2 * self.lmbda * e_dot + self.lmbda * self.lmbda * e
        e2  = e_dot + 2 * self.lmbda * e + self.lmbda * self.lmbda * self.e_int

        # 4) update parameter estimates
        self.J_hat += Ts * self.gamma1 * e2 * e1
        self.B_hat += Ts * self.gamma2 * e2 * meas_dot
        self.d_hat -= Ts * self.gamma3 * e2

        # 4b) clamp each estimate to its bounds (if set)
        if self.J_min is not None:
            self.J_hat = max(self.J_min, self.J_hat)
        if self.J_max is not None:
            self.J_hat = min(self.J_max, self.J_hat)

        if self.B_min is not None:
            self.B_hat = max(self.B_min, self.B_hat)
        if self.B_max is not None:
            self.B_hat = min(self.B_max, self.B_hat)

        if self.d_min is not None:
            self.d_hat = max(self.d_min, self.d_hat)
        if self.d_max is not None:
            self.d_hat = min(self.d_max, self.d_hat)

        # 5) compute control moment
        u = (self.J_hat * e1
             + self.B_hat * meas_dot
             + self.K * e2
             - self.d_hat)
        ...
        self.prev_e_dot = e_dot

        return u, self.J_hat, self.B_hat, self.d_hat

# Cross configuration
def rotate_z(vec, angle_deg):
    """
    Rotate a 3D vector around the Z-axis by angle_deg (degrees).
    vec: (x, y, z)
    angle_deg: rotation angle in degrees (positive = CCW looking down +Z)
    ...
    #sqrt2_inv = 1 / math.sqrt(2)  # This is √2 / 2
    #x_new = sqrt2_inv * x + sqrt2_inv * y
    #y_new = -sqrt2_inv * x + sqrt2_inv * y

    return (x_new, y_new, z)

def calibrate_imu_cross(imu, calibration_samples, dt, angle_deg=45):
    """
    Calibrate the IMU in cross configuration
    by first rotating each sensor reading
    around Z by angle_deg, then computing biases.
    """
    bx = by = bz = 0.0
    ...
    for i in range(calibration_samples):
        # read raw plus-frame data
        m9a, m9g, m9m = imu.getMotion9()
        # rotate into cross frame
        ax, ay, az = rotate_z(m9a, angle_deg)
        gx, gy, gz = rotate_z(m9g, angle_deg)
        mx, my, mz = rotate_z(m9m, angle_deg)
...
def estimate_mounting_angle(imu, samples=200, Ts = 0.005):
    sum_mx = sum_my = 0.0
    for _ in range(samples):
        _, _, (mx, my, mz) = imu.getMotion9()
        sum_mx += mx
        sum_my += my
        time.sleep(Ts)
    mx_avg = sum_mx / samples
    my_avg = sum_my / samples
    # this is the heading of the sensor-X axis relative to north
    heading_sensor_x = math.degrees(math.atan2(my_avg, mx_avg))
    # since your quad’s front is 0°, the board is rotated by this negative amount
    return -heading_sensor_x
...
Phi_KP, Phi_KI, Phi_KD = 4, 1, 2
Theta_KP, Theta_KI, Theta_KD =   4, 1, 2
...
total_simulation_time = 100 #200 # Simulation time in seconds  

# ---------------------- Get Initial Attitude from Accelrometer and Mag ---------------------- #
get_init_att_samples = 10

#angle_deg = estimate_mounting_angle(imu)
# print("Using mounting offset:", angle_deg, "degrees")


angle_deg = -45 
# Calibrate IMU and get biases
# bx, by, bz, phi_bias, theta_bias, psi_bias, m9a, m9g, m9m = calibrate_imu(imu, get_init_att_samples, Ts)
bx, by, bz, phi_bias, theta_bias, psi_bias, m9a, m9g, m9m = calibrate_imu_cross(imu, get_init_att_samples, Ts, angle_deg)
...
def main():
    ...
    for i in range(calibration_samples):
        m9a, m9g, m9m = imu.getMotion9()
        ax, ay, az = rotate_z(m9a, angle_deg) # for cross config
        gx, gy, gz = rotate_z(m9g, angle_deg) # for cross config
        mx, my, mz = rotate_z(m9m, angle_deg) # for cross config
        m9a, m9g, m9m = (ax, ay, az), (gx, gy, gz), (mx, my, mz) # for cross config
    ...
    # phi
    adaptive_phi_control  = AdaptiveAttitudeController(lmbda = 2.0,
                                    gamma1 = 1,
                                    gamma2 = 1,
                                    gamma3 = 1,
                                    K = 2,
                                    ...
                                    B_init = 0,
                                    d_init = 0,
                                    J_min   = 0.05 * Jx,   # lower bound on J_hat
                                    J_max   = 5 * Jx,   # upper bound on J_hat
                                    B_min   = -0.1,        # lower bound on B_hat
                                    B_max   = 0.1,        # upper bound on B_hat
                                    d_min   = -0.5,      # lower bound on d_hat
                                    d_max   =  0.5 )      # upper bound on d_hat)
    # theta
    adaptive_theta_control  = AdaptiveAttitudeController(lmbda = 2.0,
                                    gamma1 = 1,
                                    gamma2 = 1,
                                    gamma3 = 1,
                                    K = 2,
                                    ...
                                    B_init = 0,
                                    d_init = 0,
                                    J_min   = 0.05 * Jy,   # lower bound on J_hat
                                    J_max   = 5 * Jy,   # upper bound on J_hat
                                    B_min   = -0.1,        # lower bound on B_hat
                                    B_max   =  0.1,        # upper bound on B_hat
                                    d_min   = -0.5,      # lower bound on d_hat
                                    d_max   =  0.5 )      # upper bound on d_hat)
    # psi
    adaptive_psi_control  = AdaptiveAttitudeController(lmbda=0.1,
                                    gamma1=0.1,
                                    gamma2=0.1,
                                    gamma3=0.1,
                                    K=0.1,
                                    ...
                                    B_init=0,
                                    d_init=0,
                                    J_min   = 0.5 * Jz,   # lower bound on J_hat
                                    J_max   = 1.5 * Jz,   # upper bound on J_hat
                                    B_min   = -0.1,        # lower bound on B_hat
                                    B_max   = 0.1,        # upper bound on B_hat
                                    d_min   = -0.5,      # lower bound on d_hat
                                    d_max   =  0.5 )      # upper bound on d_hat)
    ...
    for i in range(num_samples):
        ...
        # 
        ### ----------------- Adaptive Controller --------------- ###
        U2, J_hat_phi, B_hat_phi, d_hat_phi = adaptive_phi_control.update(
            meas = phi_meas, meas_dot = phi_dot_meas,
            des = phi_des, des_dot = phi_dot_des, des_ddot = phi_ddot_des, 
            Ts = Ts )   
        
        U3, J_hat_theta, B_hat_theta, d_hat_theta = adaptive_theta_control.update(
            meas = theta_meas, meas_dot = theta_dot_meas,
            des = theta_des, des_dot = theta_dot_des, des_ddot = theta_ddot_des, 
            Ts = Ts )   

        _, J_hat_psi, B_hat_psi, d_hat_psi = adaptive_psi_control.update(
            meas = psi_meas, meas_dot = psi_dot_meas,
            des = psi_des, des_dot = psi_dot_des, des_ddot = psi_ddot_des, 
            Ts = Ts ) 
        ...
        _, U3, U4 = rate_PID(p_desired, q_desired, r_desired, p_meas, q_meas, r_meas, P_KP, P_KI, P_KD, Q_KP, Q_KI, Q_KD, R_KP, R_KI, R_KD, Ts, PQR_PID_Enable)
        # U2, U3, U4
        U4 = 0 
        # U3 = 0 
        # U2 = 0 
        ...
        storage['phi_adaptive_est_history'][i] = [J_hat_phi, B_hat_phi, d_hat_phi]
        storage['theta_adaptive_est_history'][i] = [J_hat_theta, B_hat_theta, d_hat_theta]
        storage['psi_adaptive_est_history'][i] = [J_hat_psi, B_hat_psi, d_hat_psi]
        ...
        f"w1:{omega_1:.0f}, w2:{omega_2:.0f} | w3:{omega_3:.0f}, w4:{omega_4:.0f} rad/s")
    ...
    # --------- stop motors first before saving data --------- #
    print("Experiment completed. Gradually reducing motor speeds to zero.") # can i move this up ater exp ended before saving data to mat?
    ...
    save_to_matfile(storage, gains, path_prefix='/home/pi/Documents/Quadcopter_Control_v2/data/results/quad_cross_adptv_')
