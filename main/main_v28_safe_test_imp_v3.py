...
# - Added loop-timing instrumentation and scheduler knobs so we can verify the 5 ms budget during lab runs.
# - Carry over the low-allocation path from v1 (preallocated buffers, reuse of numpy arrays).
# - Tweak PRINT_INTERVAL, SPIN_SLEEP_MARGIN_NS, WARMUP_DURATION, LOOP_OVERRUN_THRESHOLD_NS before flight tests.
...
LOOP_OVERRUN_THRESHOLD_NS = int(0.0005 * 1e9)  # Flag frames that miss the deadline by >0.5 ms
MOTOR_SAT_HALT_TOL = 1.0  # rad/s margin before treating a motor as saturated
...
def rate_PID(p_desired, q_desired, r_desired, p_meas, q_meas, r_meas, 
    ...
    if PQR_PID_Enable:
        inv_Ts = 1.0 / Ts  # reuse inverse inside hot PID loop
        # Roll Rate PID Controller
        p_error = p_desired - p_meas
        p_error_sum += p_error * Ts
        p_error_dot = (p_error - previous_p_error) * inv_Ts
        ...
        q_error_dot = (q_error - previous_q_error) * inv_Ts
        ...
        r_error_dot = (r_error - previous_r_error) * inv_Ts
...
# ---------------------- Simulation settings ---------------------- #
Ts = 0.005  # Sampling time in seconds (5 ms)\nTs_INV = 1.0 / Ts  # Precompute inverse for hot-path divisions
...
def main():
    ...
    worst_frame_ns = 0
    max_frame_ns = 0
    overrun_threshold_ns = Ts_ns + LOOP_OVERRUN_THRESHOLD_NS  # deadline incl. slack

    saturation_tripped = False
    saturation_iteration = -1
    saturation_time = 0.0
    saturation_omegas = None
    ...
    for i in range(num_samples):
        ...
        z_dot_est = alpha_dot_est * (z_dot_est + (z_ddot_meas - g) * Ts) + (1.0 - alpha_dot_est) * ((z_meas - z_est) * Ts_INV)
        z_est += z_dot_est * Ts
        ...
        # Safety check: abort once any motor hits its calibrated maximum
        saturation_now = np.any(omega_array >= (max_motor_speed - MOTOR_SAT_HALT_TOL))
        if saturation_now and not saturation_tripped:
            saturation_tripped = True
            saturation_iteration = i
            saturation_time = sim_time
            saturation_omegas = omega_array.copy()
            print(f"Motor saturation reached at t={sim_time:.2f}s (iteration {i}). Initiating shutdown.")

        if not saturation_tripped:
        ...
        if frame_time_ns > max_frame_ns:
            max_frame_ns = frame_time_ns
        if frame_time_ns > overrun_threshold_ns:
            overrun_count += 1
            if frame_time_ns > worst_frame_ns:
                worst_frame_ns = frame_time_ns
        ...
        if saturation_tripped:
            break
        wait_until(next_loop_time_ns, spin_margin_ns)

    led.setColor('Blue')

    loop_target_ms = Ts * 1e3
    print(f"Loop max frame: {max_frame_ns / 1e6:.3f} ms (target {loop_target_ms:.3f} ms)")
    if saturation_tripped:
        print(f"Experiment aborted after motor saturation at t={saturation_time:.2f}s (iteration {saturation_iteration}).")
    if overrun_count:
        slack_ms = LOOP_OVERRUN_THRESHOLD_NS / 1e6
        print(f"Loop overruns: {overrun_count} frames beyond {slack_ms:.3f} ms slack (worst {worst_frame_ns / 1e6:.3f} ms).")
    else:
        print("Loop overruns: none beyond configured slack.")

    print("Experiment completed. Gradually reducing motor speeds to zero.")
    omega_end = saturation_omegas if saturation_omegas is not None else omega_array.copy()
