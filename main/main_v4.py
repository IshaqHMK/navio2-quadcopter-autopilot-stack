# IH 2025
# version 4
# Note:  Check if enough time has passed to read the altitude again (half sampling time) - NO GOOD PROGRESS
...
madgwick = Madgwick(beta=0.033)  # Adjust the beta value as needed for responsiveness
...
def main():
    ...
    while time.time() - start_no_speed_time < 5:
        ...
        # Note: No Interchanging needed! 8-1-2025
        # -----------------------------
        time.sleep(Ts)
    ...
    # Time tracking for the last altitude reading
    last_altitude_time = time.perf_counter()
    ...
    for i in range(num_samples):
        ...
        # check if half sampling time has passed to read the altitude again # ADDED
        if time.perf_counter() - last_altitude_time >=  (Ts / 2): # ADDED
            # Read altitude from Lidar
            Z_meas = read_lidar(ser)
            if Z_meas is not None:
                Z_meas -= alt_bias  # Subtract bias to make initial point exactly 0
                if abs(Z_meas - last_valid_altitude) <= delta_altitude_threshold:
                    last_valid_altitude = Z_meas  # Update last valid altitude
                last_altitude_time = time.perf_counter()  # Update the timestamp for the last valid measurement
            else:
                # Use previous measurement if no new data
                Z_meas = last_valid_altitude
        else:
            # Use previous measurement if not enough time has passed
            Z_meas = last_valid_altitude
        ...
        # Step 5. Apply Speeds to Quadrotor
