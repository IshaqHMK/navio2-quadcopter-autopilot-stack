# IH 2025
# version 2
# Note: testing improved samplingh time 
#
...
def send_speed_command(pwm_outputs, omega, MIN_PULSE_LENGTH, MAX_PULSE_LENGTH):
    ...
    # Ensure omega is not below MIN_OMEGA
    # omega = np.maximum(omega, MIN_OMEGA) # DONT DO THIS
    # Convert omega to pulse lengths
...
MIN_PULSE_LENGTH = np.array([1.189, 1.1894, 1.189, 1.1847]) # (DO NOT CHANGE)
MAX_PULSE_LENGTH = np.array([1.753, 1.7534, 1.767, 1.75]) # (DO NOT CHANGE)
# Measured using tachometer
MIN_OMEGA_RPM = np.array([4030, 4030, 4030, 4030]) # rpm
MAX_OMEGA_RPM = np.array([21000, 21000, 21000, 21000]) # rpm

# Convert RPM to rad/s
MIN_OMEGA = (MIN_OMEGA_RPM/2) * ((2 * np.pi) / 60) # rad/s
MAX_OMEGA = (MAX_OMEGA_RPM/2) * ((2 * np.pi) / 60)# rad/s
...
m = 1.2 #1.0#  1.4   # Quadrotor mass(kg)
g = 9.80665  # Gravity (m/s^2)
l = .25 # .56   # Distance from the center of mass to the each motor(m)
KT = 1.3328e-5;    # Thrust force coeffecient (kg-m)
Kd = 1.3858e-6;    # Drag torque coeffecient (kg-m^2)
max_motor_speed_rpm = 11840  # rpm ~~ 14.8V x 800 rpm/V
...
# print after interchanging to match our system: 
print(f"Phi (Acc): {np.rad2deg(theta_bias):.2f}°, Theta (Acc): {np.rad2deg(phi_bias):.2f}°")

# Madgwick Filter Parameter
madgwick = Madgwick(beta=0.044)  # Adjust the beta value as needed for responsiveness
madgwick = Madgwick(beta=0.046)  # Adjust the beta value as needed for responsiveness
madgwick = Madgwick(beta=0.048)  # Adjust the beta value as needed for responsiveness
madgwick = Madgwick(beta=0.10)  # Adjust the beta value as needed for responsiveness
...
def main():
    ...
    # Initialize state variables for altitude integration:
    ...
    # Use accelration measurements to start from its current state:
    prev_phi = phi_bias
    prev_theta = theta_bias
    prev_psi = 0
    
    # Initialize IMU madgwick related variables
    ...
    # Color white:  All initialization completed
    ...
    while time.time() - start_no_speed_time < 5:
        ...
        #
        # 1.1.1 Read altitude from Lidar
        ...
        #
        # BEGIN get current IMU Measurements ------------------------------
        # 
        ...
        madgwick.q0 = q
        #
        # Interchange to match our confg. and process (does NOT involve MF): (before interchanging)
        ...
        #
        # END of get current IMU Measurements -----------------------------
        #
        time.sleep(Ts)
        
        
    # Set Initial Theta Value (exp will start from current position)
    ...
    # Arrays to store mesurements: 
    num_samples = int(total_simulation_time / Ts)
    sim_times = np.zeros(num_samples)
    omegas = np.zeros((num_samples, 4))  # 4 motors
    altitudes = np.zeros((num_samples, 3))  # Store altitude data
    acc_data = np.zeros((num_samples, 3))  # Accelerometer data (ax, ay, az)
    gyro_data = np.zeros((num_samples, 3))  # Gyroscope data (p, q, r)
    mag_data = np.zeros((num_samples, 3))  # Magnetometer data (mx, my, mz)
    attitude_data = np.zeros((num_samples, 3))  # Attitude data (phi, theta, psi)
    control_input_data = np.zeros((num_samples, 4))  # Control Input data (U1, U2, U3, U4)
    reference_data = np.zeros((num_samples, 4))  # Reference data (Z, phi, theta, psi)
    theta_pid_data = np.zeros((num_samples, 3))  # Theta PID data (Theta_KP, Theta_KI, Theta_KD)
    rate_reference_data = np.zeros((num_samples, 3))  # rate_reference data (p_desired, q_desired, r_desired)
    
    # --------------------------------------------------------------- #
    # --------------------------------------------------------------- #
    
    # start_time = time.time() # for simulation time - REPLACED
    # last_time = start_time # for execution time - REPLACED

    start_time = time.perf_counter()  # High-resolution timer for simulation time
    last_time = start_time  # Initialize last_time for execution timing
    ...
    for i in range(num_samples):
        ...
        val_min =0.9    #13.8 #1.6805 #13.8272  # 22  #4.1#  4.5 #2.8945 #3.1# 2.5  # 2
        val_max =1.12  #  2.8945  #2.7 # 
        time_min = 6
        time_max = total_simulation_time #80
        ...
        # 1.1.1 Read altitude from Lidar
        Z_meas = read_lidar(ser)  # upside down since positive Z is down and TFmini gives you positive up
        if Z_meas is not None:
            Z_meas -= alt_bias  # Subtract bias to make initial point exactly 0
            if abs(Z_meas - last_valid_altitude) <= delta_altitude_threshold:
                last_valid_altitude = Z_meas  # Update last valid altitude
        ...
        #
        #
        # 1.1.3 Use a complementary filter for vertical speed and position
        ...
        # 3.1 Tune Rate Controller: (comment when done)
        # p_desired, q_desired, r_desired = rate_reference_generator(sim_time) # COMMENTED
        
        # 4. Implement Rate Controller (if enabled)
        ...
        # 7. Store variables:
        altitudes[i] = [Z_meas, Z_est, Z_dot_est]  
        acc_data[i] = [X_ddot_meas, Y_ddot_meas, Z_ddot_meas]  
        gyro_data[i] = [p_meas, q_meas, r_meas]  
        mag_data[i] = [mx, my, mz]  
        attitude_data[i] = [phi_meas, theta_meas, psi_meas]  
        control_input_data[i] = [U1, U2, U3, U4]
        reference_data[i] = [Z_des_GF, phi_des, theta_des, psi_des]
        omegas[i, :] = omega  
        theta_pid_data = [Theta_KP, Theta_KI, Theta_KD]
        rate_reference_data[i] = [p_desired, q_desired, r_desired] 
        
        # Measure execution time for this iteration
        execution_time = time.perf_counter() - last_time
        
        # Print on screen Z_meas
        ...
        last_time = current_time # for next iteration
        ...
        while time.time() - last_time < Ts:
            pass  
        # Wait until the next sample time using precise timing
        next_time = last_time + Ts
        while time.perf_counter() < next_time:
          pass
        last_time = next_time  # Update for the next iteration

            
        # --------------------------------------------------------------- #
            
    # EXPERIMENT COMPLETED - Blue Color LED
    led.setColor('Blue')  # (1, 1, 0)
    
    # Define PID gains for saving:
    ...
    P_PID_Gains = {'P_KP': P_KP, 'Psi_KI': P_KI, 'P_KD': P_KD}
    Q_PID_Gains = {'Q_KP': Q_KP, 'Psi_KI': Q_KI, 'P_KD': Q_KD}
    R_PID_Gains = {'R_KP': R_KP, 'Psi_KI': R_KI, 'P_KD': R_KD}
        
    # Save Data
    # Define the path with the current date and time
    now = datetime.now()
    date_time_str = now.strftime("%d_%m_%y_%H_%M_%S")  # Updated format for better readability
    path = f'/home/pi/Documents/Quadcopter_Control_v2/data/results/quad_exp_J_on_Y_{date_time_str}.mat'
    ...
    'control_input': control_input_data,
    'reference': reference_data,
    'Z_PID_Gains': Z_PID_Gains,
    'Phi_PID_Gains': Phi_PID_Gains,
    'Theta_PID_Gains': Theta_PID_Gains,
    'Psi_PID_Gains': Psi_PID_Gains,
    'P_PID_Gains': P_PID_Gains,
    'Q_PID_Gains': Q_PID_Gains,
    'R_PID_Gains': R_PID_Gains,
    'rate_reference': rate_reference_data
    })
    ...
    #--------------------------------------------------------------
    altitude_reference = reference_data[:, 0]  #   the 1 column is altitude Z
    altitude_measured = altitudes[:, 0]    #   the 0 column is Z measured
    # Call the function to display metrics for Phi control  (EXTERNAL FUNCTION)
    display_controller_metrics(sim_times, altitude_reference, altitude_measured, omegas, sim_time, Ts, "Altitude Control", time_min, time_max)
    #--------------------------------------------------------------
    phi_reference = reference_data[:, 1]  #   the 1 column is theta_des
    phi_measured = attitude_data[:, 0]    #   the 0 column is theta_meas 
    # Call the function to display metrics for Phi control  (EXTERNAL FUNCTION)
    display_controller_metrics(sim_times, np.rad2deg(phi_reference), np.rad2deg(phi_measured), omegas, sim_time, Ts, "Phi Control", time_min, time_max)
    #--------------------------------------------------------------
    # Extracting specific columns for theta control analysis
    theta_reference = reference_data[:, 2]  #   the third column is theta_des
    theta_measured = attitude_data[:, 1]    #   the third column is theta_meas
    # Call the function to display metrics for Theta control  (EXTERNAL FUNCTION)
    display_controller_metrics(sim_times, np.rad2deg(theta_reference), np.rad2deg(theta_measured), omegas, sim_time, Ts, "Theta Control", time_min, time_max)
    #--------------------------------------------------------------
    psi_reference = reference_data[:, 3]  #   the 1 column is theta_des
    psi_measured = attitude_data[:, 2]    #   the 0 column is theta_meas 
    # Call the function to display metrics for Phi control  (EXTERNAL FUNCTION)
    display_controller_metrics(sim_times, np.rad2deg(psi_reference), np.rad2deg(psi_measured), omegas, sim_time, Ts, "Psi Control", time_min, time_max)
    #
    plot_results(sim_times, reference_data, altitudes[:, 0], attitude_data, control_input_data)
    #--------------------------------------------------------------
    #display_controller_metrics(sim_times, np.rad2deg(rate_reference_data[:, 0]), np.rad2deg(gyro_data[:, 0]), omegas, sim_time, Ts, "p Control")
    #display_controller_metrics(sim_times, np.rad2deg(rate_reference_data[:, 1]), np.rad2deg(gyro_data[:, 1]), omegas, sim_time, Ts, "q Control")
    ...
    for pwm_out in pwm_outputs:
       pwm_out.disable()  # Disable PWM outputs on exit
